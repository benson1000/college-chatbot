import numpy as np                  #used in tensorflow for training and  multi-dimensional arrays and matrices, along with a collection of functions for performing mathematical operations on these arrays.
import random                 #for choice a random answer from dataset
...
def chat(model,bag_of_words,labels,words,data):  
    ...
    print("Hi, How can i help you ?")       #starting message or the bot
    while True:                             #while asking msg
        ...
        results_index = np.argmax(results)                  #convert results to numpy array 
        ...
        if results[results_index] > 0.8:         #if the result or prediction more than 80 % ,The prediction is displayed as an output on the chat utility window as a response from the bot
            ...
            sleep(2)                         # take 2 sec for response
            Bot = random.choice(responses)   #choice random response from dataset
            print(Bot)                       #print response 
        else:
            print("I don't understand!")     #if the user type any msg that the bot not understand         